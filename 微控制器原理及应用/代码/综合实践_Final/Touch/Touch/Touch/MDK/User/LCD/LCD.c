...
/*******************************************************************************
* 函数名称：LCD_CtrlLinesConfig
* 描述：以复用功能配置 LCD 控制线（FSMC 引脚）推挽模式。
* 输入：无
* 输出：无
* 返回：无
* 注意：无
...
static void LCD_CtrlLinesConfig(void)
{
	...
	/* 启用 FSMC、GPIOD、GPIOE、GPIOF、GPIOG 和 AFIO 时钟 */
	RCC_AHB3PeriphClockCmd(RCC_AHB3Periph_FSMC, ENABLE); 
	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOD | RCC_AHB1Periph_GPIOE , ENABLE);
	
	/* 设置PD.00(D2), PD.01(D3), PD.04(NOE), PD.05(NWE), PD.08(D13), PD.09(D14),PD.10(D15), PD.11(A16), PD.14(D0), PD.15(D1) 作为复用功能推挽 */
	...
	/*设置PE.07（D4）、PE.08（D5）、PE.09（D6）、PE.10（D7）、PE.11（D8）、PE.12（D9）、PE.13（D10）,PE.14（D11）、PE.15（D12）作为复用功能推拉*/
...
/*******************************************************************************
* 函数名称：LCD_FSMCConfig
* 描述 : 为 LCD(Parallel mode) 配置并行接口 (FSMC)
* 输入：无
* 输出：无
* 返回：无
* 注意：无
...
void LCD_Initializtion(void)
{
	...
	/* 设置 PLL 的 MN(multipliers)，VCO = 晶振频率 * (N+1) */
	/* PLL 频率 = VCO/M，250MHz < VCO < 800MHz */
	/* 最大 PLL 频率约为 120MHz。 获得 120MHz 作为 PLL 频率 */ 
...
* 说明：			此功能利用ssd1963的PWM特性来调整背光强度。				
*输入：			-强度：强度来自0x00（总背光关断，PWM引脚下拉至VSS---0xff（99% 上拉，255/256 上拉至 VDD）                              
* 输出：			无
* 返回：			无
* 注意：			PWM 的基频设置为 300Hz 左右，PLL 设置为 120MHz。此参数取决于硬件。带关机引脚的背光电路连接到 ssd1963 的 PWM 输出。
*******************************************************************************/
...
/*******************************************************************************
* 函数名：		LCD_Clear
* Description : 将屏幕填充成指定的颜色，如屏，则填充0xffff
* 输入：			 - 颜色：屏幕颜色
* 输出：			无
* 返回：			无
* 注意：			无
...
/******************************************************************************
* 函数名称：		LCD_BGR2RGB
* Description : RRRRRGGGGGGBBBBB改为BBBBBGGGGGGRRRRR格式
* 输入：-颜色：	BRG 颜色值
* 输出：			无
* 返回：			RGB 颜色值
* 注意：			内部函数调用
