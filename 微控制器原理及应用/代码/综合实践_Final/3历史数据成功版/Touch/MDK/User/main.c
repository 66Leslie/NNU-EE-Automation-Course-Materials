...
u8 Key_read(void)
{
	...
	if(WarningFlag == 1)
	{
		if(display.x > 220 && display.x < 320 && display.y > 180 && display.y < 240) return 5; //确认
		return 0;
	}
	...
	if(uiPage == 1)
	{
		/* 右上角返回：x=240~319, y=0~40 */
		if(display.x > 240 && display.x < 320 && display.y > 0 && display.y < 40) return 10;
		return 0;
	}

/* 主界面：启动/停止、正转/反转、调速模式（旋钮调速开关） */
	if(display.x > 120 && display.x < 220 && display.y > 120 && display.y < 180) return 2; //正转
	if(display.x > 120 && display.x < 220 && display.y > 180 && display.y < 240) return 3; //反转

		/* 右侧两块区域：上方=旋钮调速开关，下方=历史数据 */
	if(display.x > 220 && display.x < 320 && display.y > 120 && display.y < 180) return 8; //旋钮调速开关
	if(display.x > 220 && display.x < 320 && display.y > 180 && display.y < 240) return 9; //历史数据


	if(display.x < 120 && display.x > 0 && display.y > 120 && display.y < 180) return 6; //启动
	if(display.x < 120 && display.x > 0 && display.y > 180 && display.y < 240) return 7; //停止
...
void Display(void)
{
	...
	if(WarningFlag == 0)       //主界面/历史界面显示
	{
		...
		GUI_Chinese(248,158, speedMode ? "开启" : "关闭", speedMode ? Green : Grey, Black);
	}
...
void Display_HistoryInit(void)
{
    ...
    /* 返回按钮（右上角） */
    GUI_Chinese(250,10,"返回",Black,Yellow);
    LCD_DrawLine(240,0,319,0,Yellow);
    LCD_DrawLine(240,40,319,40,Yellow);
    LCD_DrawLine(240,0,240,40,Yellow);
    LCD_DrawLine(319,0,319,40,Yellow);
    ...
    GUI_Char(300,214,"t",White,Black);
}
...
void History_DrawPlot(void)
{
    ...
    {
        /* 不清右上角“返回”按钮区域（0~40） */
        LCD_FillRect_ByLine(0, 45, 319, 239, Black);
    ...
    for(i=0;i<5;i++)
    {
        ...
        /* Y轴刻度数值（摄氏度，1位小数） */
        {
            ...
            /* 左侧对齐显示（预留足够空间） */
            GUI_Char(5, (y>8)?(y-8):y, s, White, Black);
        }
    }
    ...
    GUI_Char(right-5, bottom+4, "t", White, Black);

    /* 右上角统计信息：Min/Max/Now（摄氏度） */
    sprintf(s, "Min:%d.%dC", (int)(tMin/10), (int)(tMin%10));
    GUI_Char(170,10,s,White,Black);
    sprintf(s, "Max:%d.%dC", (int)(tMax/10), (int)(tMax%10));
    GUI_Char(170,25,s,White,Black);
    sprintf(s, "Now:%d.%dC", (int)(tNow/10), (int)(tNow%10));
    GUI_Char(170,40,s,White,Black);
    sprintf(s, "N=%d", cnt);
    GUI_Char(265,40,s,White,Black);
