...
class TrackingMixedResetMotionCommand(MotionCommand):
  ...
  def _resample_home_command(self, env_ids: torch.Tensor) -> None:
    ...
    if self.cfg.home_root_position_mode == "motion_xyz":
      reference_pos = self.motion.body_pos_w[start_frame, 0].unsqueeze(0).expand(
        len(env_ids), -1
      )
      ...
      raise ValueError(
        "Unsupported home_root_position_mode: "
        f"{self.cfg.home_root_position_mode}"
      )
    root_pos += pose_noise[:, :3] + self._env.scene.env_origins[env_ids]
    if self.cfg.home_root_orientation_mode == "motion":
      reference_ori = self.motion.body_quat_w[start_frame, 0].unsqueeze(0).expand(
        len(env_ids), -1
      )
      ...
      raise ValueError(
        "Unsupported home_root_orientation_mode: "
        f"{self.cfg.home_root_orientation_mode}"
      )
